...
app.controller('MatchCtrl', function(Match, Auth, uid, profile, $scope, $ionicModal, Like, Messages, $ionicScrollDelegate, $timeout) {
	...
	matc.openMessageModal = function(matchUid) {
		Auth.getProfile(matchUid).$loaded().then(function(profile) {
			matc.currentMatchUser = profile;
			Messages.historyMessages(matchUid, uid).$loaded().then(function(data) {
				matc.messages = data;
				$scope.modal.show();
				$timeout(function() {
